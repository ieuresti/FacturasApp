...
public class FacturaXlsxView extends AbstractXlsxView {
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
		...
		// Obtenemos el obj de la factura que nos estan enviando
		Factura factura = (Factura) model.get("factura");
		Sheet sheet = workbook.createSheet("Factura Spring"); // Creamos la hoja
		
		MessageSourceAccessor mensajes = getMessageSourceAccessor();
		
		Row row = sheet.createRow(0); // 1ra fila
		Cell cell = row.createCell(0); // 1ra columna
		...
		for (ItemFactura item : factura.getItems()) {
			...
			cell.setCellValue(item.getProducto().getNombre()); // Crear la celda por cada atributo
